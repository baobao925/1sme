...
import { Wallet, Banknote, ReceiptText, Check, X, Eye, Info, ShieldAlert, Calendar, ListOrdered } from "lucide-react";
...
const Payouts = () => {
  ...
  const [commissionDetail, setCommissionDetail] = useState<PayoutRequest | null>(null);
  ...
  return (
    <TooltipProvider delayDuration={200}>
                ...
                return (
                  <tr key={p.id} className="border-t border-border hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 text-xs leading-relaxed">
                      {p.mappedCommissions && p.mappedCommissions.length > 0 ? (
                        <button
                          onClick={() => setCommissionDetail(p)}
                          className="text-left group"
                        >
                          <div className="space-y-1">
                            {p.mappedCommissions.map(m => (
                              <div key={m.orderCode} className="whitespace-nowrap group-hover:text-primary transition-colors">
                                <span className="font-medium text-foreground group-hover:text-primary underline decoration-dotted underline-offset-2">{m.orderCode}</span>: {formatVND(m.mappedAmount)} / <span className="text-muted-foreground">{formatVND(m.totalAmount)}</span>
                              </div>
                            ))}
                          </div>
                          <div className="mt-1 text-[10px] text-primary opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                            <ListOrdered className="w-3 h-3" /> Xem chi tiết hoa hồng
                          </div>
                        </button>
                      ) : (
      ...
      {/* Dialog Chi tiết Hoa hồng Tương ứng */}
      <Dialog open={!!commissionDetail} onOpenChange={(o) => !o && setCommissionDetail(null)}>
        <DialogContent className="w-[95vw] max-w-[1440px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ListOrdered className="w-5 h-5 text-primary" />
              Chi tiết Hoa hồng Tương ứng
            </DialogTitle>
            <DialogDescription>{commissionDetail?.code} · Kỳ {commissionDetail?.period}</DialogDescription>
          </DialogHeader>
          {commissionDetail && (
            <div className="space-y-4">
              {/* Thông tin chung Affiliate */}
              <div className="bg-muted/40 rounded-lg p-3 grid grid-cols-2 gap-x-6 gap-y-1.5 text-sm border border-border">
                <div className="col-span-2 text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Thông tin chung</div>
                <Row label="Mã Affiliate" value={<span className="font-mono font-semibold">{commissionDetail.affiliateCode || commissionDetail.affiliateId}</span>} />
                <Row label="Tên Affiliate" value={commissionDetail.affiliateName} />
              </div>

              {/* Bảng tóm tắt đơn hàng */}
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Tóm tắt thông tin đơn hàng</div>
              {commissionDetail.mappedCommissions && commissionDetail.mappedCommissions.length > 0 ? (
                <div className="overflow-x-auto rounded-lg border border-border">
                  <table className="w-full text-xs min-w-[1100px]">
                    <thead className="bg-muted/60 text-[10px] uppercase text-muted-foreground">
                      <tr>
                        <th className="text-center px-2 py-2 font-semibold w-8">STT</th>
                        <th className="text-left px-2 py-2 font-semibold whitespace-nowrap">Mã đơn hàng</th>
                        <th className="text-left px-2 py-2 font-semibold min-w-[140px]">Tên sản phẩm</th>
                        <th className="text-center px-2 py-2 font-semibold whitespace-nowrap">Ngày click</th>
                        <th className="text-center px-2 py-2 font-semibold whitespace-nowrap">Ngày đặt đơn</th>
                        <th className="text-center px-2 py-2 font-semibold whitespace-nowrap">Ngày HT</th>
                        <th className="text-right px-2 py-2 font-semibold whitespace-nowrap">Tổng DT</th>
                        <th className="text-right px-2 py-2 font-semibold whitespace-nowrap">% HH</th>
                        <th className="text-right px-2 py-2 font-semibold whitespace-nowrap">Tổng HH đơn</th>
                        <th className="text-right px-2 py-2 font-semibold whitespace-nowrap">Đã rút</th>
                        <th className="text-right px-2 py-2 font-semibold whitespace-nowrap">Rút lần này</th>
                        <th className="text-right px-2 py-2 font-semibold whitespace-nowrap">Còn lại</th>
                        <th className="text-center px-2 py-2 font-semibold whitespace-nowrap">Trạng thái</th>
                        <th className="text-center px-2 py-2 font-semibold">FIFO</th>
                      </tr>
                    </thead>
                    <tbody>
                      {commissionDetail.mappedCommissions.map((m, idx) => (
                        <tr key={m.orderCode} className="border-t border-border hover:bg-muted/20">
                          <td className="px-2 py-2 text-center text-muted-foreground">{idx + 1}</td>
                          <td className="px-2 py-2 font-mono font-semibold text-foreground whitespace-nowrap">{m.orderCode}</td>
                          <td className="px-2 py-2 text-foreground">{m.productName}</td>
                          <td className="px-2 py-2 text-center text-muted-foreground whitespace-nowrap">{formatDate(m.clickAt)}</td>
                          <td className="px-2 py-2 text-center text-muted-foreground whitespace-nowrap">{formatDate(m.orderedAt)}</td>
                          <td className="px-2 py-2 text-center text-muted-foreground whitespace-nowrap">{m.completedAt ? formatDate(m.completedAt) : <span className="italic">Chưa HT</span>}</td>
                          <td className="px-2 py-2 text-right whitespace-nowrap">{formatVND(m.orderValue)}</td>
                          <td className="px-2 py-2 text-right font-semibold">{m.commissionRate}%</td>
                          <td className="px-2 py-2 text-right font-semibold">{formatVND(m.totalAmount)}</td>
                          <td className="px-2 py-2 text-right text-muted-foreground whitespace-nowrap">{m.alreadyWithdrawn > 0 ? formatVND(m.alreadyWithdrawn) : "—"}</td>
                          <td className="px-2 py-2 text-right font-bold text-primary whitespace-nowrap">{formatVND(m.mappedAmount)}</td>
                          <td className="px-2 py-2 text-right whitespace-nowrap">{m.remaining > 0 ? formatVND(m.remaining) : <span className="text-muted-foreground">—</span>}</td>
                          <td className="px-2 py-2 text-center">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                              m.withdrawnStatus === "Đã rút hết"
                          ...
                          <td className="px-2 py-2 text-center">
                            <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-primary/10 text-primary font-bold text-[10px]">{m.fifoIndex}</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="border-t-2 border-border bg-muted/30">
                      <tr>
                        <td colSpan={8} className="px-2 py-2 text-right text-xs font-semibold text-muted-foreground">Tổng cộng:</td>
                        <td className="px-2 py-2 text-right text-xs font-bold">{formatVND(commissionDetail.mappedCommissions.reduce((s, m) => s + m.totalAmount, 0))}</td>
                        <td className="px-2 py-2 text-right text-xs text-muted-foreground">{formatVND(commissionDetail.mappedCommissions.reduce((s, m) => s + m.alreadyWithdrawn, 0))}</td>
                        <td className="px-2 py-2 text-right text-xs font-bold text-primary">{formatVND(commissionDetail.mappedCommissions.reduce((s, m) => s + m.mappedAmount, 0))}</td>
                        <td className="px-2 py-2 text-right text-xs">{formatVND(commissionDetail.mappedCommissions.reduce((s, m) => s + m.remaining, 0))}</td>
                        <td colSpan={2} />
                      </tr>
                    </tfoot>
                  </table>
                ...
                <p className="text-sm text-muted-foreground italic">Không có dữ liệu hoa hồng tương ứng.</p>
              )}
            </div>
          )}
        </DialogContent>
