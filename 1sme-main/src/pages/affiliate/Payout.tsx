...
import { Wallet, Coins, CheckCircle2, Receipt, X, Info, AlertTriangle, ShieldAlert, Eye } from "lucide-react";
...
const Payout = () => {
  ...
  const [historyDetail, setHistoryDetail] = useState<PayoutRequest | null>(null);
  ...
  return (
    <TooltipProvider delayDuration={200}>
      ...
      {/* Dialog Chi tiết lịch sử yêu cầu */}
      <Dialog open={!!historyDetail} onOpenChange={(o) => !o && setHistoryDetail(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Chi tiết yêu cầu rút tiền</DialogTitle>
            <DialogDescription>
              {historyDetail?.code} · Kỳ {historyDetail?.period}
            </DialogDescription>
          </DialogHeader>
          {historyDetail && (
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-3">
                <DetailRow label="Mã yêu cầu" value={<span className="font-mono text-xs">{historyDetail.code}</span>} />
                <DetailRow label="Kỳ thanh toán" value={historyDetail.period} />
                <DetailRow label="Ngày tạo" value={formatDate(historyDetail.createdAt)} />
                <DetailRow label="Ngày thanh toán" value={historyDetail.paidAt ? formatDate(historyDetail.paidAt) : "—"} />
              </div>

              <div className="border-t border-border pt-3 space-y-1.5">
                <DetailRow label="Số tiền yêu cầu" value={<strong>{formatVND(historyDetail.requestedAmount)}</strong>} />
                <DetailRow label="Tổng HH trong kỳ" value={formatVND(historyDetail.periodCommission)} />
                <DetailRow
                  label={`Khấu trừ thuế (${historyDetail.taxType === "None" ? "Miễn" : historyDetail.taxType + " 10%"})`}
                  value={
                    historyDetail.taxAmount > 0
                      ? <span className="text-destructive font-semibold">-{formatVND(historyDetail.taxAmount)}</span>
                      : <span className="text-muted-foreground">0 đ</span>
                  }
                />
                {historyDetail.serviceFee > 0 && (
                  <DetailRow label="Phí dịch vụ" value={<span className="text-muted-foreground">-{formatVND(historyDetail.serviceFee)}</span>} />
                )}
                <DetailRow
                  label="Thực nhận"
                  value={<span className="text-primary font-bold text-base">{formatVND(historyDetail.net)}</span>}
                />
              </div>

              <div className="border-t border-border pt-3">
                <DetailRow label="Tài khoản nhận" value={<span className="font-mono text-xs">{historyDetail.bankAccount}</span>} />
                <div className="mt-2">
                  <DetailRow label="Trạng thái" value={<StatusBadge status={historyDetail.status} />} />
                </div>
              </div>

              {/* Lý do từ chối */}
              {historyDetail.rejectReason && (
                <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-3 space-y-1">
                  <div className="text-xs font-semibold text-destructive">Lý do từ chối</div>
                  <div className="text-sm">{historyDetail.rejectReason}</div>
                </div>
              )}
              ...
              {historyDetail.note && (
                <div className="bg-muted/40 rounded-lg p-3">
                  ...
                  <div className="text-sm">{historyDetail.note}</div>
                </div>
              )}
            </div>
          )}
